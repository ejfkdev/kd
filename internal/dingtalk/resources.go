...
func (d *Dumper) downloadOne(ctx context.Context, it *resourceItem) resourceRecord {
	...
	var lastErr error
	// 1) DingTalk media download (media_id body) — only for media resources
	if mid := strVal(it.Extra["media_id"]); mid != "" {
		if data, err := d.do(ctx, callSpec{method: "GET", path: "/media/download", query: map[string]string{"media_id": mid}, raw: true}); err == nil {
...
// sanitizeName mirrors the feishu tool's naming hygiene.
